...
/**
 * Created by Weiting on 2017/9/14.
 */
/**
...
import com.example.weiting.bluetooth.R;
...
public class BluetoothScanner {
    ...
    boolean found = false;
    int TX_POWER = 60;
    int count2 = 0;//判斷前面放棄幾筆資料
    int count = 0;//佇列中有幾筆值
    int check_count = 0;//判斷被慮掉了幾筆值
    int rssiAverge = 0;
    int rssiAverge2 = 0;
    int pastRssi = 0;
    ...
    public BluetoothScanner(Context context, ListView listView, TextView view,TextView view2)
    {
                    ...
                    if (patternFound) {
                        ...
                        if(major == 04 && minor == 28) {
                           ...
                           if(count < 10){
                                ...
                                rssiAverge = rssiAverge / 8;

                                if(rssi - (int)rssiArray2.getLast().getValue() < 8 && (rssi - (int)rssiArray2.getLast().getValue()) > -8){ //新讀到的值差8以上
                                    rssiArray2.addLast(rssi);
                                    rssiArray2.removeFirst();

                                }/*else{//排除10筆以上時無條件更新佇列值
                                    check_count++;
                                    if(check_count%20 == 0){
                                        rssiArray2.addLast(rssi);
                                        rssiArray2.removeFirst();
